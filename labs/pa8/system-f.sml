    ...
    case (t1, t2)
     of (Bool, Bool) => true
      ...
      | (Arrow (a1, b1), Arrow (a2, b2)) =>
        equals (a1, a2) andalso equals (b1, b2)
      | (Product (t1, t2), Product (s1, s2)) =>
        equals (t1, s1) andalso equals (t2, s2)
      | (Forall (x, t1), Forall (y, t2)) =>
        x=y andalso equals(t1, t2)
    ...
    let fun doit (G, t) =
            case t
             of Bool => Bool
              ...
              | Product (t1, t2) =>
                Product (doit (G, t1), doit (G, t2))
              | Forall (x, t) =>
                let val new = fresh ()
                in  Forall (new, doit (fn z =>
                                          if z=x
                                          then new
                                          else G z
                                     , t))
...
fun subst (x, t1, t2) =
    case t2
     of Bool => Bool
      ...
      | Product (s1, s2) =>
        Product (subst (x, t1, s1), subst (x, t1, s2))
      | Forall (x, t') =>
        let val (y, t'') =
                case alpha (t2)
                 of Forall (y, t'') => (y, t'')
...
fun isFree (x, t) =
    case t
     of Bool => false
      ...
      | Arrow (t1, t2) =>
        isFree (x, t1) orelse isFree (x, t2)
      | Product (t1, t2) =>
        isFree (x, t1) orelse isFree (x, t2)
      | Forall (y, t') =>
        let val (z, t'') =
...
val t1 = App (TyApp (t11, Type.Arrow(Type.Bool, Type.Bool))
            , TyApp (t11, Type.Bool))
val _ = (pp t1; print "\n")
...
val t2 = Pack (Type.Bool, Pair (True, Abs ("x", Type.Bool, Var "x"))
              , Type.Exists ("X"
                           , Type.Product (Type.Var "X"
                                         , Type.Arrow (Type.Var "X",
                                                       Type.Var "X"))))
...
(* Type error! *)
val t3 = Unpack ("X", "x", t2, App (Second (Var "x")
                                  , First (Var "x")))
